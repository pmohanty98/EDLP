...
import GWG_release.toy_data
import torchvision
import torchvision.transforms as tr
from torch.utils.data import DataLoader, TensorDataset
import numpy as np
import GWG_release.visualize_flow
import matplotlib.pyplot as plt
import pickle
import GWG_release.rbm
import GWG_release.samplers
...
def approx_difference_function(x, model):
    ...
    if x.grad is None:
        ...
        x.grad = grad.detach().clone()
        #print(x.grad)
    gx = x.grad * 2. #NOTE: workaround for Binary NNs  #torch.autograd.grad(model(x).sum(), x)[0]
    wx = gx * -(2. * x - 1)
    return wx.detach()
'''
