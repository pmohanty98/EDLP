...
def get_sampler(args):
    ...
    if args.input_type == "binary":
        if args.sampler == "gibbs":
            ...
            sampler = samplers.ELangevinSampler(data_dim, 1,
                                           fixed_proposal=False, approx=True, multi_hop=False, temp=2., alpha=args.step_size, alpha_a=args.step_size_a,eta=args.eta, mh=True)

        elif args.sampler == "edula":
            sampler = samplers.ELangevinSampler(data_dim, 1,
                                           fixed_proposal=False, approx=True, multi_hop=False, temp=2., alpha=args.step_size, alpha_a=args.step_size_a,eta=args.eta, mh=False)
        elif args.sampler == "edmala-glu":
            sampler = samplers.ELangevinSamplerwithGLU(data_dim, 1,
                                                fixed_proposal=False, approx=True, multi_hop=False, temp=2.,
                                                alpha=args.step_size, eta=args.eta, mh=True)

        elif args.sampler == "edula-glu":
            sampler = samplers.ELangevinSamplerwithGLU(data_dim, 1,
                                                fixed_proposal=False, approx=True, multi_hop=False, temp=2.,
                                                alpha=args.step_size, eta=args.eta, mh=False)
        elif args.sampler == 'egdmala':
            sampler = samplers.EGLangevinSampler(data_dim, 10,
                                                 fixed_proposal=False, approx=True, multi_hop=False, temp=2.,
                                                 alpha=args.step_size, mh=True, eta=args.eta, score_sweeps=4)
        elif args.sampler == 'egdula':
            sampler = samplers.EGLangevinSampler(data_dim, 10,
                                                 fixed_proposal=False, approx=True, multi_hop=False, temp=2.,
                                                 alpha=args.step_size, mh=False, eta=args.eta, score_sweeps=4)
        else:
...
def main(args):
    ...
    my_print(sampler)
    my_print(str(args.step_size)+" "+str(args.step_size_a)+" "+str(args.eta))
        ...
        for x in train_loader:
            ...
            for k in range(args.sampling_steps):
                if sampler.entropy==False:
                    ...
                    sample_tuple = sampler.step(x_fake.detach(), x_fake_a.detach(), model)
                    x_fake_new = sample_tuple[0].detach()
                    x_fake_a_new = sample_tuple[1].detach()
                h = (x_fake_new != x_fake).float().view(x_fake_new.size(0), -1).sum(-1).mean().item()
                hops.append(h)
                x_fake = x_fake_new
                if sampler.entropy==True:
                    x_fake_a = x_fake_a_new
            st = time.time() - st
            ...
            if (itr + 1) % args.eval_every == 0:
                ...
                if val_ll.item() > 0:
                    print(f"Exiting early at iteration {itr} with val_ll = {val_ll.item()}")
                    exit()
                if val_ll.item() > best_val_ll:
                    best_val_ll = val_ll.item()
                    my_print("Best valid likelihood")
                    torch.save(d, "{}/best_ckpt_{}_{}_{}_{}_{}.pt".format(args.save_dir,args.dataset_name,args.sampler,args.step_size, args.step_size_a,args.eta))
                else:
                    torch.save(d, "{}/ckpt_{}_{}_{}_{}_{}.pt".format(args.save_dir,args.dataset_name,args.sampler,args.step_size,args.step_size_a,args.eta))

                model.to(device)

            itr += 1
    np.save("{}/test_ll_{}_{}_{}.npy".format(args.save_dir,args.dataset_name,args.sampler,args.step_size),test_ll_list)
    print(args.sampler,args.step_size, args.step_size_a, args.eta)
if __name__ == "__main__":
    ...
    parser.add_argument('--buffer_size', type=int, default=10000) # all datasets except Caltech
    # mcmc
    ...
    parser.add_argument('--step_size_a', type=float, default=0.001)
